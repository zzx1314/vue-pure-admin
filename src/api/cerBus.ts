...
export const getProjList = () => {
  return http.axiosGetRequest<Result>(otaDevUrls.getProjList, null);
};
// OTACa获取子证书列表
...
// OTACa根据id查询
export const getById = (param?: number) => {
  return http.axiosGetRequest<ResultOne>(otaDevUrls.getById + param, null);
};
type DownParam = {
  id: number;
  type: string;
