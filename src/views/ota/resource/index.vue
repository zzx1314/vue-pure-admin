...
const uploadFile = async (index, item) => {
  addForm.value.originFileName = item.name;
  ...
  // 进度按完整文件大小计算，断点续传时不会因为只统计待上传分片而超过100%。
  const totalSize = item.size;

  // plimit 并发上传
  const uploadLimit = needUploadFile.map(n =>
    limit(() => uploadChunkUrl(n, index, item.size, item.file.type, item.md5))
  );
...
const initSliceFile = async (item, initData) => {
  ...
  const code = result.code;
  const data = result.data;
  if (code !== 10200 || !data) return [];

  // 后端负责生成并保存 uploadId，前端只保留分片序号和二进制内容。
  const uploadedParts = new Set(listParts || []);
  item.chunkFileList.forEach((chunk, index) => {
    const partNumber = index + 1;
    if (!uploadedParts.has(partNumber)) {
      needUploadFile.push({ partNumber, file: chunk });
    }
  });
...
const uploadChunkUrl = (
  ...
  return new Promise((resolve, reject) => {
    uploadMultipartPart(md5, chunkItem.partNumber, chunkItem.file, type)
      .then(result => {
