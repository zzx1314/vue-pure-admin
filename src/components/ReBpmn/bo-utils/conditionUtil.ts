...
import { getEventDefinition } from "@/components/ReBpmn/utils/BpmnImplementationType";
import modeler from "@/store/modeler";
import { createModdleElement } from "@/components/ReBpmn/utils/BpmnExtensionElementsUtil";
...
export function isCanbeConditional(element: BpmnElement): boolean {
  return (
    (is(element, "bpmn:SequenceFlow") &&
      isConditionalSource((element as Connection)?.source)) ||
...
export function getVariableNameValue(element: Element): string | undefined {
  if (getConditionalEventDefinition(element)) {
    return (getConditionalEventDefinition(element) as ModdleElement).get(
      "variableName"
    );
  }
}
...
// 4. 元素条件表达式
export function getConditionExpressionValue(
  element: Element
): string | undefined {
  const conditionExpression = getConditionExpression(element);
  if (conditionExpression) {
...
export function getConditionScriptTypeValue(
  ...
  if (conditionExpression.get(`${prefix}:resource`) !== undefined)
    return "external";
  return "none";
}
...
// 6. 元素脚本 语言类型
export function getConditionScriptLanguageValue(
  element: Element
): string | undefined {
...
// 7. 元素脚本 body
export function getConditionScriptBodyValue(
  element: Element
): string | undefined {
...
export function getConditionTypeOptions(
  ...
  return defaultConditionTypeOptions.filter(
    condition => condition.value !== "default"
  );
}
...
function getConditionExpression(
  ...
  if (getConditionalEventDefinition(businessObject)) {
    return (getConditionalEventDefinition(businessObject) as ModdleElement).get(
      "condition"
    );
  }
}
...
function setDefaultCondition(element: Connection, isDefault: boolean) {
  ...
  if (element.source) {
    // 确保 element.source 是 bpmn-js 定义的 Element 类型
    const sourceElement: Element = {
      ...element.source,
      businessObject: getBusinessObject(element.source),
      di: element.source.di || {},
      type: element.source.type || "bpmn:Unknown"
    };
    // 更新默认条件
    ...
    console.warn("Source element is undefined, cannot set default condition.");
  }
}
