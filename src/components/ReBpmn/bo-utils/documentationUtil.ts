...
export function getDocumentValue(element: Element): string {
  ...
  const documentation =
    businessObject && findDocumentation(businessObject.get("documentation"));
...
export function setDocumentValue(element: Element, value: string | undefined) {
  ...
  const bpmnFactory: BpmnFactory | undefined =
    store.getModeler?.get("bpmnFactory");
...
function findDocumentation(docs) {
  return docs.find(function (d) {
    return (
      (d.textFormat || DOCUMENTATION_TEXT_FORMAT) === DOCUMENTATION_TEXT_FORMAT
    );
  });
}
