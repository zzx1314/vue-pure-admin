  ...
  >
    ...
    <template v-if="item.type === 'object'">
      <div v-if="model === undefined">
        <span class="dynamic-form-error-alert">
          [object] 警告：输入字段 {{ name }} 是 undefined</span
        >
      </div>
      <template v-else>
        <!--标题-->
        ...
        >
          <template #insertion>
            <span class="dynamic-form-object-title">{{
              evaluateCallback(item.label)
            }}</span>
          </template>
        </DynamicFormItemNormal>
        <!--循环子条目-->
    ...
    <FormGroup
      ...
      :title="evaluateCallback(item.label)"
      v-bind="item.additionalProps"
    >
      <div v-if="model === undefined">
        <span class="dynamic-form-error-alert">
          [group-object] 警告：输入字段 {{ name }} 是 undefined</span
        >
      </div>
    ...
    <!--扁平组-->
    <FormGroup
      v-else-if="item.type === 'group-flat'"
      :title="evaluateCallback(item.label)"
      v-bind="item.additionalProps"
    >
    ...
    <!--扁平普通-->
    <DynamicFormItemNormal
      v-else-if="item.type === 'simple-flat'"
      :item="item"
      :name="name"
      :disabled="disabled || evaluateCallback(item.disabled)"
...
export default defineComponent({
  ...
  components: { DynamicFormItemNormal, FormGroup, FormArrayGroup, Row, Col }
});
</script>
