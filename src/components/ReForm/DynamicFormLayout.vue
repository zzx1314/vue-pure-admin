...
import DynamicFormInner from "./DynamicFormInnerLayout.vue";
...
import {
  IDynamicFormMessageCenter,
  IDynamicFormMessageCenterCallback
} from "./DynamicFormInternal";
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const messageCenterMap = new Map<
      string,
      IDynamicFormMessageCenterCallback
    >();
    ...
    provide("messageCenter", {
      addInstance: (name: string, fn: IDynamicFormMessageCenterCallback) =>
        messageCenterMap.set(name, fn),
    ...
    function accessFormModel(
      ...
      while (obj) {
        ...
        if (leftIndex > 0 && key.endsWith("]")) {
          const arr = obj[key.substring(0, leftIndex)] as Record<
            string,
            unknown
          >[];
    ...
    return () => {
      const renderChildren = () =>
        h(
          DynamicFormInner as any,
          { options: finalOptions.value, model: model.value },
          ctx.slots
        );
      ...
      if (internalWidgetsForm) {
        console.log("From", formRules);
        return h(
          internalWidgetsForm.component as any,
          {
            ...props,
            ref: formEditor,
            [internalWidgetsForm.propsMap.rules || "rules"]: formRules,
            [internalWidgetsForm.propsMap.model || "model"]: model.value,
            [internalWidgetsForm.propsMap.labelWidth || "labelWidth"]:
              formLabelWidth,
            [internalWidgetsForm.propsMap.labelCol || "labelCol"]: formLabelCol,
            [internalWidgetsForm.propsMap.wrapperCol || "wrapperCol"]:
              formWrapperCol,
            [internalWidgetsForm.propsMap.onFinish || "onFinish"]: (
              d: unknown
            ) => ctx.emit("finish", d),
            [internalWidgetsForm.propsMap.onFinishFailed || "onFinishFailed"]: (
              d: unknown
            ) => ctx.emit("finishFailed", d),
            [internalWidgetsForm.propsMap.onSubmit || "onSubmit"]: (
              d: unknown
            ) => ctx.emit("submit", d),
            ...formAdditionaProps,
            ...formAdditionalEvents
...
<style lang="scss">
@import url("./Scss/BaseControl.scss");
@import url("./Scss/Form.scss");
