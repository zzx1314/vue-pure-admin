  ...
  <div class="dynamic-form-array-group">
    <div v-if="model === undefined">
      <span class="dynamic-form-error-alert">
        [array] 警告：输入字段 {{ name }} 是 undefined</span
      >
    </div>
    <!--循环数据条目-->
    <!--对象类型-->
    <template v-else-if="isObject">
      <div v-for="(childData, key) in model" :key="key" class="item-container">
        <FormArrayGroupItem
          :item="item"
          :name="name"
          :childData="childData"
    ...
    <slot name="addButton" :onClick="handleAdd">
      <button
        v-if="showAddButton"
        class="add dynamic-form-base-control base-button"
        type="button"
        @click="handleAdd"
      >
        + 添加
  ...
  methods: {
    handleAdd() {
      if (typeof this.addCallback === "function") {
        ...
        if (typeof ret !== "undefined") arr.push(ret);
      }
    },
    ...
    handleUp(data: unknown) {
      const arr = this.model;
      const index = arr.indexOf(data);
      if (index > 0) ArrayUtils.upData(arr, index);
    },
    handleDown(data: unknown) {
      const arr = this.model as unknown[];
      const index = arr.indexOf(data);
      if (index < arr.length - 1) ArrayUtils.downData(arr, index);
    }
  },
